...
std::string read_file(const std::string& filename)
{
    ...
    // Confirm the file opened successfully
    if (!file.is_open()) {
    ...
    // Read the file line by line into the file_text string
    std::string line;
...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
  ...
  //  Line 2: timestamp (yyyy-mm-dd)
    ...
    if (localtime_s(&datetime, &timestamp) == 0) {
        file << std::put_time(&datetime, "%Y/%m/%d") << "\n";
    }
    else {
...
int main()
{
  ...
  const std::string file_name = "C:\\Users\\dcava\\source\\repos\\M5 Encryption\\x64\\Debug\\inputdatafile.txt";
